...
NzbDocument = Fn.inherits('Informer', function NzbDocument(filelink, server) {
	...
	// Set the yenc throttle to 35ms (pause between yenc decodes)
	this.yenqueue.throttle = 35;
 ...
 * @param    {Function}   fnc
 */
NzbDocument.setMethod(function readyForPar(file) {
	...
	this.getTemp('repair', function gotPath(err, path) {
		...
		var parfiles = [that.pars.main].concat(that.pars.others),
		    filetasks,
		    tasks,
		    file,
		    key;
	...
	doPar = function doPar() {
		...
		fixer = Blast.Classes.Nitpin.repair(that.pars.main.parpath);
