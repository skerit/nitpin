...
NitpinWorker = Blast.Collection.Function.inherits('Informer', function NitpinWorker(parent) {
	...
	var that = this,
	    sockType = require(this.secure ? 'tls' : 'net'),
	    attempts = 0,
	    socket;
	...
	// New queue
	this.comboqueue = Fn.createQueue();

	// Socket communication queue
	this.commqueue = Fn.createQueue();
 ...
 * @version  0.1.4
 */
NitpinWorker.setMethod(function debug() {
 ...
 * @version  0.1.3
 */
NitpinWorker.setMethod(function destroy() {
...
NitpinWorker.setMethod(function _submit(command, multiline, compressed, callback) {
	...
	this.commqueue.add(function doCommand(done) {
		...
		// Create a timebomb: explode when we haven't called back in 1 minute
		bomb = Fn.timebomb(60000, finish);
